...
lamda=[1,1,1,1]
Equation_name=f'Burgers_shock_{x_num}_out'
...
if MODEL=='Train':
    ...
    for iter in tqdm(range(iter_num)):
        ...
        #                f'model_save/{Equation_name}/optimizer_save_{data_num}_{noise_level}.pkl')
            ...
            # plt.show()
    torch.save(Net.state_dict(),
               f'model_save/{Equation_name}/model_save_{data_num}_{noise_level}_only_data.pkl')
...
if MODEL=='Test':
    ...
    prediction_ori=Net(database_ori).reshape(nx_ref,nt_ref).cpu().data.numpy()
    prediction = Net(DR_valid).cpu().data.numpy()
    loss_valid = np.mean((Y_valid.cpu().data.numpy() - prediction) ** 2)
    prediction_ref=Net(database_ref).cpu().data.numpy()
    loss_ref=np.mean((y_ref.cpu().data.numpy() - prediction_ref) ** 2)
    print(loss_ref)
    print(loss_valid)

    plt.imshow(get_plot(prediction_ori),cmap='coolwarm',vmin=-1,vmax=1)
    plt.axis('off')
    plt.colorbar()
    plt.show()
    plt.imshow(get_plot(un_ref-prediction_ori),cmap='coolwarm',vmin=-0.6,vmax=0.6)
    plt.axis('off')
    plt.colorbar()
    plt.show()
