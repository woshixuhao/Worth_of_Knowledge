...
lamda=[1,1,1,1]
Equation_name=f'Convection_diffusion_{x_num}'
print(f'data_num:{data_num}, noise: {noise_level}')
#====referenced data==========
data_path = f'Data/CD.mat'
data_ref = scio.loadmat(data_path)
un_ref = data_ref.get("u").T
...
def initial():
    ...
    ref_ini =torch.sin(math.pi*x_ini)*torch.exp(-x_ini)
...
class Loss_func():
    ...
    def loss_inter(self,database_inter):
        ...
        loss=torch.mean((Ht+Hx-0.25*Hxx)**2)
        return loss
