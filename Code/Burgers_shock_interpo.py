...
lamda=[1,1,1,1]
Equation_name=f'Burgers_shock_{x_num}'
print(f'data_num:{data_num}, noise: {noise_level}')
#====referenced data==========
data_path = f'Data/burgers_shock.mat'
data_ref = scio.loadmat(data_path)
un_ref = data_ref.get("usol")



x_ref = np.squeeze(data_ref.get("x"))
t_ref = np.squeeze(data_ref.get("t"))
...
def get_plot(un):
    ...
    t_num=un.shape[1]
    total=x_num*t_num
    un_plot=np.zeros([x_num,2*t_num])
...
# plt.imshow(get_plot(un_ref),cmap='coolwarm')
# plt.axis('off')
# plt.colorbar()
# plt.show()
try:
...
def initial():
    ...
    ref_ini =-torch.sin(math.pi*x_ini)
...
class Loss_func():
    ...
    def loss_inter(self,database_inter):
        ...
        loss=torch.mean((Ht+H*Hx-(0.01/math.pi)*Hxx)**2)
        return loss
    ...
    def loss_bl(self, database_bl, ref_bl):
        prediction = Net(database_bl)
        loss = torch.mean((prediction-ref_bl)**2)
        return loss

    def loss_br(self,database_br, ref_br):
        prediction = Net(database_br)
        loss = torch.mean((prediction-ref_br)**2)
        return loss
...
Y_valid=torch.clone(y_ref[valid_index])  #Causion!!
...
if MODEL=='Train':
    ...
    #     torch.load(f'model_save/{Equation_name}/optimizer_save_{data_num}_{noise_level}.pkl'))
    for iter in tqdm(range(iter_num)):
        optimizer.zero_grad()
        l_inter=Loss.loss_inter(database_interior)
        l_ini=Loss.loss_ini(database_ini,ref_ini)
        l_bl=Loss.loss_bl(database_bl,ref_bl)
        l_br=Loss.loss_br(database_br,ref_br)
        l_data=Loss.loss_data(DR_train,Y_train)

        if data_num==0:
            loss = l_inter + l_ini + l_bl + l_br
        else:
            if iter < 8000:
                loss = l_data
            if iter>=8000:
                loss=l_inter + l_ini + l_bl + l_br+l_data

        rule_loss=l_inter + l_ini + l_bl + l_br
        data_loss=l_data
        loss.backward()
        optimizer.step()
        ...
        if (iter + 1) % 1 == 0:
            ...
            rule_loss_record.append(rule_loss.cpu().data.numpy())
            data_loss_record.append(data_loss.cpu().data.numpy())
            #print(f'loss:   {loss.cpu().data.numpy()}, loss_valid:{loss_valid}')
            # plt.plot(loss_record)
            # plt.plot(loss_valid_record,c='red')
            # plt.show()
    torch.save(Net.state_dict(),
               f'model_save/{Equation_name}/model_save_{data_num}_{noise_level}.pkl')
    np.save(f'result_save/{Equation_name}/loss_record.npy',loss_record)
    np.save(f'result_save/{Equation_name}/loss_valid.npy', loss_valid_record)
    np.save(f'result_save/{Equation_name}/rule_loss_record.npy', rule_loss_record)
    np.save(f'result_save/{Equation_name}/data_loss_valid.npy', data_loss_record)
...
if MODEL=='Test':
    ...
    loss_record=np.load(f'result_save/{Equation_name}/loss_record.npy')[1::100]
    loss_valid_record=np.load(f'result_save/{Equation_name}/loss_valid.npy')[1::100]
    rule_loss_record=np.load(f'result_save/{Equation_name}/rule_loss_record.npy')[1::100]
    data_loss_record = np.load(f'result_save/{Equation_name}/data_loss_valid.npy')[1::100]

    plt.figure(1,figsize=(4,3),dpi=300)
    plt.style.use('ggplot')
    plt.yscale('log')
    plt.plot(loss_record,label='Train loss')
    plt.plot(loss_valid_record,c='#8A83B4',label='Valid loss')
    plt.xticks(np.arange(0,loss_valid_record.shape[0]+1,50),np.arange(0,loss_valid_record.shape[0]+1,50)*100,fontproperties='Arial', size=8)
    plt.yticks(fontproperties='Arial', size=8)
    plt.legend(prop=font1)
    plt.savefig(f'fig_save/{Equation_name}/loss_{data_num}_{noise_level}.tiff', bbox_inches='tight', dpi=300)
    plt.savefig(f'fig_save/{Equation_name}/loss_{data_num}_{noise_level}.pdf', bbox_inches='tight', dpi=300)
    plt.savefig(f'fig_save/{Equation_name}/loss_{data_num}_{noise_level}.jpg', bbox_inches='tight', dpi=300)
    plt.show()

    plt.figure(2, figsize=(4, 3), dpi=300)
    plt.style.use('ggplot')
    plt.yscale('log')
    plt.plot(rule_loss_record, label='Rule loss')
    plt.plot(data_loss_record, c='#8A83B4', label='Data loss')
    plt.xticks(np.arange(0,loss_valid_record.shape[0]+1,50),np.arange(0,loss_valid_record.shape[0]+1,50)*100,fontproperties='Arial', size=8)
    plt.yticks(fontproperties='Arial', size=8)
    plt.legend(prop=font1)
    plt.savefig(f'fig_save/{Equation_name}/rule_data_{data_num}_{noise_level}.tiff', bbox_inches='tight', dpi=300)
    plt.savefig(f'fig_save/{Equation_name}/rule_data_{data_num}_{noise_level}.pdf', bbox_inches='tight', dpi=300)
    plt.savefig(f'fig_save/{Equation_name}/rule_data_{data_num}_{noise_level}.jpg', bbox_inches='tight', dpi=300)
    plt.show()
    ...
    noise_level=0
    Net.load_state_dict(
        torch.load(f'model_save/{Equation_name}/model_save_0_0.pkl'))

    prediction=Net(database_ref).reshape(nx_ref,nt_ref).cpu().data.numpy()
    print(np.mean((un_ref-prediction)**2))
    plt.figure(1,dpi=300)
    plt.imshow(get_plot(un_ref),cmap='coolwarm',vmin=-1,vmax=1)
    plt.rcParams['font.family'] = 'Arial'
    if Equation_name == 'KG_equation':
        plt.rcParams['font.size'] = 8
    plt.axis('off')
    plt.colorbar()
    plt.savefig(f'fig_save/{Equation_name}/ref_{data_num}_{noise_level}.pdf', bbox_inches='tight', dpi=300)
    plt.show()
            ...
            for bit_3 in [0, 1]:
                for bit_4 in [0, 1]:
                    ...
                    if data_num==0:
                        ...
                        optimizer.load_state_dict(
                            torch.load(f'model_save/{Equation_name}/optimizer_save_pretrain_{data_num}_{noise_level}.pkl'))
                    for iter in tqdm(range(measure_num)):
                        optimizer.zero_grad()
                        reg_control = [bit_1, bit_2, bit_3, bit_4]
                        if data_num==0:
                            if reg_control == [0, 0, 0, 0]:
                    ...
                    prediction=Net(DR_valid).cpu().data.numpy()
                    loss_valid=np.mean((Y_valid.cpu().data.numpy()-prediction)**2)
                    MSE_record[bit_1, bit_2, bit_3, bit_4] = loss_valid
...
if MODEL=='SHAP':
    ...
    def plot_noise_trend():
        ...
        ax=plt.axes()
        color_box = ['#2878b5', '#838AB4', '#f8ac8c', '#c82423', '#ff8884']
        x_plot=np.array([0,10,20,30,40,50])
        reg_box = ['Rule 1', 'Rule 2', 'Rule 3', 'Rule 4']
        for i in range(4):
            plt.plot(All_shap[i], marker='x', color=color_box[i], label=reg_box[i])
            print(i, All_shap[i])
        plt.xticks(np.arange(0,6,1),x_plot/100,fontproperties='Arial', size=7)
        plt.yticks(fontproperties='Arial', size=7)
        plt.legend(prop=font1, loc='lower left')
        #plt.ylim(-0.5, 1.5)
        plt.ylim(-2, 2)
        ...
        plt.savefig(f'fig_save/{Equation_name}/noise_trend_reg_plot_rule.tiff', bbox_inches='tight', dpi=300)
        plt.savefig(f'fig_save/{Equation_name}/noise_trend_reg_plot_rule.jpg', bbox_inches='tight', dpi=300)
        plt.savefig(f'fig_save/{Equation_name}/noise_trend_reg_plot_rule.pdf', bbox_inches='tight', dpi=300)
        plt.show()
    ...
    def plot_meta_num():
        noise_level = 0
        All_shap = []
        for meta_num in [10, 50, 100, 200, 500]:
            result = np.load(f'result_save/Burgers_shock_{meta_num}/MSE_data_{data_num}_{noise_level}.npy')
            shap_value = PINN_SHAP(result, plot_pic=False)
            All_shap.append(shap_value)
        All_shap = np.array(All_shap).T
        for i in range(All_shap.shape[0]):
        ...
        plt.style.use('ggplot')
        color_box = ['#2878b5', '#838AB4', '#f8ac8c', '#c82423', '#ff8884']
        x_plot = ['$10^2$', '$50^2$', '$100^2$', '$200^2$', '$500^2$']
        reg_box = ['Rule 1', 'Rule 2', 'Rule 3', 'Rule 4']
        for i in range(4):
            plt.plot(All_shap[i], marker='x', color=color_box[i], label=reg_box[i])
        plt.xticks(np.arange(0, 5, 1), x_plot, fontproperties='Arial', size=7)
        ...
        plt.ylim([-0.5,1.5])
        plt.savefig(f'fig_save/{Equation_name}/data_meta.tiff', bbox_inches='tight', dpi=300)
        plt.savefig(f'fig_save/{Equation_name}/data_meta.jpg', bbox_inches='tight', dpi=300)
        plt.savefig(f'fig_save/{Equation_name}/data_meta.pdf', bbox_inches='tight', dpi=300)
        plt.show()
