...
Equation_name=f'KG_equation_{x_num}_out_pred'
lamda=[1,1,1,1,1]
...
def initial():
    ...
    ref_ini =torch.exp(-20*x_ini*x_ini)*(torch.sin(math.pi*x_ini)+torch.sin(math.pi*2*x_ini))
...
class Loss_func():
    ...
    def loss_inter(self,database_inter):
        ...
        Htt = torch.autograd.grad(outputs=Ht.sum(), inputs=database_inter, create_graph=True)[0][:, 1].reshape(-1, 1)
        loss=torch.mean((Htt+5*H-0.5*Hxx)**2)
        return loss
    ...
    def loss_ini_t(self,database_ini,ref_ini):
        prediction = Net(database_ini)
        H_grad_ini = torch.autograd.grad(outputs=prediction.sum(), inputs=database_ini, create_graph=True)[0]
        Ht_ini = H_grad_ini[:, 1].reshape(-1, 1)
        loss = torch.mean((Ht_ini)**2)
        return loss
...
def PINN_SHAP(result,plot_pic=True):
    ...
    if plot_pic==True:
        ...
        plt.yticks(np.arange(0,len(plot_array)),['reg1','reg2','reg3','reg4','reg5'],fontproperties='Arial',size=7)
...
if MODEL=='Train':
    for iter in tqdm(range(iter_num)):
        ...
        l_ini_t=Loss.loss_ini_t(database_ini,ref_ini)
        l_data=Loss.loss_data(DR_train,Y_train)
        if data_num!=0:
            loss=l_inter+l_ini+l_bl+l_br+l_ini_t+l_data
        if data_num==0:
            loss = l_inter+l_ini+l_bl+l_br+l_ini_t
        loss.backward()
        optimizer.step()
        if (iter + 1) % 1000 == 0:
            prediction = Net(database_ref).cpu().data.numpy()
            loss_valid=np.mean((y_ref.cpu().data.numpy()- prediction) ** 2)
            prediction_ref=Net(database_ori).reshape(nx_ref, nt_ref).cpu().data.numpy()
            # plt.figure()
            # plt.imshow(prediction_ref)
            # plt.show()
            # plt.clf()
            ...
            for bit_3 in [0, 1]:
                for bit_4 in [0, 1]:
                    for bit_5 in [0, 1]:
                        ...
                        prediction = Net(DR_valid).cpu().data.numpy()
                        loss_valid = np.mean((Y_valid.cpu().data.numpy() - prediction) ** 2)
                        MSE_record[bit_1, bit_2, bit_3, bit_4,bit_5] = loss_valid
...
if MODEL=='SHAP':
    def plot_data_trend():
        ...
        reg_box = ['reg1', 'reg2', 'reg3', 'reg4','reg5']
        for i in range(5):
